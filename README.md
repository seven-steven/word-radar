...
- Node.js ≥ 20
- pnpm ≥ 9
- Chrome 127+ 或 Edge 127+（用于加载扩展；右键菜单入口依赖 127 起开放的 `chrome.action.openPopup`，见 `docs/adr/0001`）
...
3. **点击扩展图标，点「采集当前页」**：点工具栏图标只打开弹窗（不自动采集，issue #39）；弹窗内点「采集当前页」按钮显式采集当前页正文，确认卡显示「本次共计采集 N 个单词，其中新词 M 个」，点「确认推送」后才合并入库；累计 / 待推计数同步刷新。
...
| 按钮                | 作用                                                    |
| ------------------- | ------------------------------------------------------- |
| 采集当前页          | 对当前标签页跑一次正文提取，先出确认卡，确认后才合并入库 |
...
| `contextMenus`                                | 仅注册两个右键菜单入口项（「采集当前页」「上传文件采集生词」），不读取页面内容；页面采集仍走 `activeTab` + `scripting` 现有路径。                                                                             |
