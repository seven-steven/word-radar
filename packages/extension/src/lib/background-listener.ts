...
import { createBbdcClient } from "./bbdc-client.js";
...
export function createBackgroundListener(deps: BackgroundListenerDeps) {
  ...
  return (
    message: unknown,
    _sender: unknown,
    sendResponse: (response: Counts | CheckLoginResponse | { ok: false; error: string }) => void,
  ): boolean => {
    ...
    if (isCheckLoginMessage(message)) {
      handleCheckLogin(bbdcClient, actionBadge)
