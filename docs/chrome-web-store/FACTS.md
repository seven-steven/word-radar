...
- 打包命令：`pnpm package` → `dist/word-radar-extension.zip`（仓库根 `dist/`，manifest 位于 zip 根层级）。
- 版本号唯一来源：根 `package.json` 的 `version`（当前 0.1.0）；上传 Dashboard 时 zip 文件名可手工带版本（如 `word-radar-0.1.0-chrome.zip`），但仓库产物名固定为 `word-radar-extension.zip`。
