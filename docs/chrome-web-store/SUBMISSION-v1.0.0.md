# SUBMISSION-v1.0.0 — Chrome Web Store 提交手册

- 版本：`1.0.0`（根 package.json / src manifest / zip 三方一致，verify:manifest ✓）
- 上传包：`dist/word-radar-1.0.0-chrome.zip`（verify:zip ✓）
- 事实基线：[FACTS.md](./FACTS.md)（verify:claims ✓）
- 截图：[screenshots/](./screenshots/) 01–03，1280×800（本次随 Berkeley Mono 视觉重生成）
- Release：https://github.com/seven-steven/word-radar/releases/tag/v1.0.0

> 字段口径以 2026-08-22 Dashboard 实测为准；详细实测 dump 与字段上下限见 [research-cws-dashboard-fields.md](./research-cws-dashboard-fields.md) 与 skill `references/cws-materials.md`。
...
1. [Developer Dashboard](https://chrome.google.com/webstore/devconsole) → WordRadar → 新版本 → 上传。
2. 选择本地文件 `dist/word-radar-1.0.0-chrome.zip`。
3. 等待 "Status: ready"。
4. 文件包 tab 自动读取（无需手填）：
   - 版本 = `1.0.0`
   - 内容类型 = `扩展程序`
...
| ------------------------------- | ------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| 软件包中的标题                  | **自动**（manifest） | manifest.name（i18n `__MSG_extName__` 解析后值；**不建议覆盖**）                                                      |
| 软件包中的摘要                  | **自动**（manifest） | manifest.description（**不建议覆盖**）                                                                                  |
| 说明                            | 手填，≤16,000        | [STORE-LISTING.md](./STORE-LISTING.md)「详细描述」text 块                                                               |
| 类别                            | 手填单选             | STORE-LISTING「类别」＝效率工具（**当前 Dashboard 实测 = 教育**；以实际选项为准）                                       |
| 语言                            | 手填单选             | 默认 = `中文（中国）`                                                                                                    |
| 商店图标 128×128                | 手填                 | `packages/extension/src/assets/icons/icon-128.png`                                                                     |
| 屏幕截图（1–5 张）              | 手填                 | `screenshots/01-reading.png`、`02-collect.png`、`03-push.png`（**JPEG 或 24 位 PNG（无 alpha）**；1280×800 或 640×400） |
...
| 隐私权政策网址*              | 2,048 必填 | `https://github.com/seven-steven/word-radar/blob/v1.0.0/docs/chrome-web-store/PRIVACY.md`                                 |
...
- [x] `pnpm verify:manifest` / `verify:zip` / `verify:claims` / `verify:changelog` 全绿（2026-09-06 发版编排已跑）
- [x] `pnpm test` / `pnpm e2e` 全绿（e2e 20/20）
- [x] 上传的 zip 即 `dist/word-radar-1.0.0-chrome.zip`（dist/ 唯一版本化 zip）
- [ ] 商品详情每个字段已按上表来源粘贴（说明字段直接复制 STORE-LISTING「详细描述」text 块，不改写）
- [ ] 隐私权字段已填：4 框权限理由（host 两条合并一框）、Remote code 选 No、9 类 Data usage 勾 2 类、Limited use 三勾、隐私政策 URL 必填
