...
- 采集确认闸门(2026-08-23 定稿):采集结果驻留内存、**不持久化**,popup 打开时对当前标签页重新采集(选区不可重放,退化为全文,接受);确认页展示「本次共计采集 total 个单词,其中新词 x 个」,新词 = 与本地词库的 lemma diff,**零网络请求**;确认 = 合并入库 + 触发一轮「推送词库全部待推」;取消 = 什么都不发生。确认即推送是唯一路径,**无自动推送开关**。新采集的待推与存量待推(如 CSV 导入)语义上不区分,同属一个待推池、同一轮推送。
...
- 推送进度实时展示:popup 订阅推送状态实时刷新,只展示汇总计数(已推送/待推/词库总词数、成功/已存在/失败),不逐词标注;失败由后台重试 + 保留待推 +「重试待推词」按钮兜底。badge:推送期间显示 `x/y` 进度,完成/失败暂停时变色回执(✓/!),零新权限。错误日志存 `chrome.storage.local` 环形缓冲(最近 ~200 条),popup 提供导出。
